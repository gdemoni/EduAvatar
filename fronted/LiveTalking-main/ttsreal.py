...
class AliyunTTS(BaseTTS):
    def __init__(self, opt, parent):
        super().__init__(opt, parent)
        self.ak_id = os.getenv("ALIYUN_ACCESS_KEY_ID")
        self.ak_secret = os.getenv("ALIYUN_ACCESS_KEY_SECRET")
        self.app_key = os.getenv("ALIYUN_APPKEY")
        self.token = os.getenv("ALIYUN_TOKEN")
        self.token_expire_time = 0
        self.voice = opt.REF_FILE or "xiaoyun"  # 默认语音
    ...
    def _get_token(self):
        ...
        if not self.ak_id or not self.ak_secret:
            logger.error("Aliyun TTS: Missing ALIYUN_ACCESS_KEY_ID or ALIYUN_ACCESS_KEY_SECRET")
            return None

        # Generate token using POP API
        try:
            params = {
            ...
            if "Token" in result:
                ...
                logger.info(f"Aliyun TTS: Token refreshed, expires at {self.token_expire_time}")
                return self.token
            else:
                logger.error(f"Aliyun TTS: Failed to get token: {result}")
                return None
        except Exception as e:
            logger.exception(f"Aliyun TTS: Token generation error: {e}")
            return None

    def _percent_encode(self, string):
        res = urllib.parse.quote_plus(str(string))
    ...
    def txt_to_audio(self, msg: tuple[str, dict]):
        ...
        if not token:
            logger.error("Aliyun TTS: No valid token")
            return

        self.stream_tts(
            self.aliyun_tts_stream(text, token),
            msg
        )

    def aliyun_tts_stream(self, text, token) -> Iterator[bytes]:
        ...
        try:
            ...
            if res.status_code != 200:
                logger.error(f"Aliyun TTS Error: {res.text}")
                return

            first = True
            for chunk in res.iter_content(chunk_size=6400): # 16k * 2 bytes * 0.2s = 6400
                if first:
                    logger.info(f"Aliyun TTS: Time to first chunk: {time.perf_counter() - start:.4f}s")
                    first = False
                if chunk and self.state == State.RUNNING:
                    yield chunk
                    
        except Exception as e:
            logger.exception(f"Aliyun TTS: Stream error: {e}")
    ...
    def stream_tts(self, audio_stream, msg: tuple[str, dict]):
        ...
        for chunk in audio_stream:
            if chunk is not None and len(chunk) > 0:
                # Aliyun returns PCM 16-bit 16000Hz directly
