...
from ttsreal import EdgeTTS,SovitsTTS,XTTS,CosyVoiceTTS,FishTTS,TencentTTS,DoubaoTTS,IndexTTS2,AzureTTS,AliyunTTS
...
class BaseReal:
    def __init__(self, opt):
        ...
        elif opt.tts == "aliyuntts":
            self.tts = AliyunTTS(opt,self)
        ...
        self.subtitle_text = ""
        try:
            self.font = ImageFont.truetype("C:/Windows/Fonts/simhei.ttf", 30)
        except Exception as e:
            logger.warning(f"Font load failed: {e}")
            self.font = None

    def put_msg_txt(self,msg,datainfo:dict={}):
        if hasattr(self, 'msg_callback') and callable(self.msg_callback):
    ...
    def process_frames(self,quit_event,loop=None,audio_track=None,video_track=None):
        ...
        while not quit_event.is_set():
            ...
            for audio_frame in audio_frames:
                _, _, eventpoint = audio_frame
                if eventpoint and eventpoint.get('status') == 'start':
                    self.subtitle_text = eventpoint.get('text', "")
            
            # Draw subtitle
            if self.subtitle_text and self.font:
                try:
                    # 去除标点符号
                    clean_text = re.sub(r'[，。、？！；：“”‘’\[\]（）…,.?!;:"\'\[\]\(\)\.]+', ' ', self.subtitle_text)
                    clean_text = re.sub(r'\s+', ' ', clean_text).strip()
                    
                    if not clean_text:
                         pass
                    else:
                        img_pil = Image.fromarray(cv2.cvtColor(combine_frame, cv2.COLOR_BGR2RGB))
                        ...
                        if lines:
                            # 获取单行高度
                            bbox = draw.textbbox((0, 0), "测试", font=self.font)
                            line_height = bbox[3] - bbox[1] + 10 # 行高 + 行间距
                            ...
                            bg_w += padding * 2
                            bg_h = total_text_height + padding * 2
                            
                            bg_x = (W - bg_w) // 2
                            bg_y = H - bg_h - 30 # 距离底部 30px
                            
                            # 绘制半透明圆角背景
                            overlay = Image.new('RGBA', img_pil.size, (0, 0, 0, 0))
                            overlay_draw = ImageDraw.Draw(overlay)
                            # Pillow 12.1.1 支持 rounded_rectangle
                            overlay_draw.rounded_rectangle([(bg_x, bg_y), (bg_x + bg_w, bg_y + bg_h)], radius=10, fill=(0, 0, 0, 100))
                            ...
                            for line in lines:
                                ...
                                text_x = (W - text_w) // 2
                                # 绘制白色文字
                                draw.text((text_x, text_y), line, font=self.font, fill=(255, 255, 255))
                                text_y += line_height
                        
                        combine_frame = cv2.cvtColor(np.array(img_pil.convert('RGB')), cv2.COLOR_RGB2BGR)
                except Exception as e:
                    logger.warning(f"Subtitle draw error: {e}")
