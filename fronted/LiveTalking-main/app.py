...
async def broadcast(text):
    for ws in ws_clients:
        try:
            await ws.send_str(json.dumps({"type": "answer", "text": text}))
        except Exception as e:
            logger.error(f"WS broadcast error: {e}")

def llm_callback(text):
    if main_loop and ws_clients:
        asyncio.run_coroutine_threadsafe(broadcast(text), main_loop)
...
async def offer(request):
    ...
    # 绑定 LLM 消息回调
    nerfreal.msg_callback = llm_callback
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tts', type=str, default='edgetts', help="tts service type") #xtts gpt-sovits cosyvoice fishtts tencent doubao indextts2 azuretts aliyuntts
    parser.add_argument('--REF_FILE', type=str, default="zh-CN-YunxiaNeural",help="参考文件名或语音模型ID，默认值为 edgetts的语音模型ID zh-CN-YunxiaNeural, 若--tts指定为azuretts, 可以使用Azure语音模型ID, 如zh-CN-XiaoxiaoMultilingualNeural, 若--tts指定为aliyuntts, 可以使用阿里云语音模型ID, 如xiaoyun")
    ...
    appasync.router.add_get("/chat_socket", chat_socket) # 新增 WebSocket 路由
